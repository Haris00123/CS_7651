...
def function_eval(fitness_func,fitness_func_name,length,algorithim,algorithim_name,hyperparameter_dict,results_dict,only_binary=True,tune=True,reps=5,graph=True,**kwargs):
        ...
        for k,v in hyperparameter_dict.items():
            ...
            for value in v:
                ...
                for ii in range(reps):
                    ...
                    #PLEASE REMEMBER TO CHANGE IN BOTH PLACES HERE AND FINAL EVAL
                    ...
                    if fitness_func_name=='MaxKColor':
                        ...
                        problem = MaxKColorGenerator().generate(seed=i*2,number_of_nodes=length, max_connections_per_node=2*length, max_colors=3)
                        init_state=None
    ...
    for i in range(reps):
        ...
        #PLEASE REMEMBER TO CHANGE IN BOTH PLACES HERE AND PARAM TUNING
            ...
            for size in size_range:
                ...
                if problem=='KnapSack':
                    ...
                    p=multi.Process(target=function_eval,args=[
                        ...
                        OPT_PROBLEMS[problem],problem,size,ALGORITHIM[algorithim],algorithim,BEST_PARAMS_OG[algorithim],results_dict,only_binary,tune],kwargs=kwargs)
                    p.start()
                    processes.append(p)
                else:
                    p=multi.Process(target=function_eval,args=[
                        ...
                        OPT_PROBLEMS[problem],problem,size,ALGORITHIM[algorithim],algorithim,BEST_PARAMS_OG[algorithim],results_dict,only_binary,tune])
                    p.start()
                    processes.append(p)
