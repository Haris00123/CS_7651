...
def create_param_curve(data,param,param_vals,ax,algorithm,default_dict,metric=accuracy_score,metric_name='Accuracy',graph=True,cv=2):
...
def deal_algorithm(data,test,param_dicts,dataset,algorithm,default_dict,metric=accuracy_score,metric_name='Accuracy',cv=5):
    ...
    print('Beginning run for  {}'.format(algorithm))
    ...
    #     best_val=create_param_curve(data,param,param_vals,ax,algorithm,default_dict,metric,metric_name)
    #     best_vals[param]=best_val

    final_dict=default_dict.copy()
    # return_dict=default_dict.copy()

    # print('Hyperparameter Tuning completed for  {}'.format(algorithm))
    ...
    folder_path='p2\\Neural_Net\{}'.format(algorithm)

    if not os.path.exists(folder_path):
        os.makedirs(folder_path)

    for i in tqdm.tqdm(range(cv)):
        ...
        pd.DataFrame(data=[learner.fitness_curve[:,0],learner.fitness_curve[:,1]]).T.rename(columns={0:'Loss',1:'Function Evals'}).to_csv('p2\\Neural_Net\\{}\\run_{}.csv'.format(algorithm,i))

    print('Completed Curves & onto LC for  {}'.format(algorithm))

    #Learning Curve
    learner=NeuralNetwork(**final_dict)
    LearningCurveDisplay.from_estimator(learner,data[0],data[1],ax=axes[-1],cv=5,train_sizes=[0.25,0.5,0.75,1])
...
if __name__ == "__main__":
    ...
    #default_dict=optimized_defaults_dict=deal_algorithm(train_standardized,test_standardized,param_dict,'Heart Dataset','gradient_descent',default_dict,cv=2)
