...
def create_param_curve(data,param,param_vals,ax,algorithm,default_dict,metric=accuracy_score,metric_name='Accuracy',graph=True,cv=5):
    ...
    metric (sklearn.metric): Metric to score algorithm on
    metric_name (str): Metric Name
    graph (bool): Bool for validation graph
    cv (int): The number of cross-validation folds
    
    Returns:
    best_val: The best parameter value found
    '''
...
def deal_algorithm(data,test,param_dicts,dataset,algorithm,default_dict,metric=accuracy_score,metric_name='Accuracy',cv=5):
    ...
    Parameters:
    data (list): List of np.arrays containing the features and labels
    test (list): List of np.arrays containing the test features and labels
    param_dicts (dict): Dictionary of parameter dictionaries to vary
    dataset (str): Dataset name
    algorithm (str): Algorithm name
    default_dict (dict): Default parameters for the algorithm
    metric (sklearn.metric): Metric to score algorithm on
    metric_name (str): Metric Name
    cv (int): The number of cross-validation folds 

    Returns:
    return_dict (dict): Dictionary of best parameter values found
    '''
...
if __name__ == "__main__":
    ...
    default_dict=deal_algorithm(train_standardized,test_standardized,param_dict,'Heart Dataset','gradient_descent',default_dict,cv=5)
